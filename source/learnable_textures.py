...
class LearnableImageFourier(LearnableImage):
    def __init__(self,
        ...
        self.features = nn.Parameter(self.feature_extractor(self.uv_grid),
                                     requires_grad=False)  # pre-compute this if we're regressing on images
...
class LearnableTexturePack(nn.Module):
    ...
    def forward(self):
        ...
        assert output.shape == (self.num_textures, self.num_channels, self.height, self.width), str(
            "WTF? " + str(output.shape) + " IS NOT " + str(
                (self.num_textures, self.num_channels, self.height, self.width)))
...
class LearnableAlphasFourier(LearnableImage):
    ...
    def forward(self, condition=None):
        ...
        assert output.shape == (self.num_channels, self.height, self.width), '%s != %s' % (
        output.shape, (self.num_channels, self.height, self.width))
